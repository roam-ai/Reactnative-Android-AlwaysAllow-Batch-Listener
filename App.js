    ...
    try {
      ...
      if (fineLocationGranted === PermissionsAndroid.RESULTS.GRANTED) {
        ...
        if (backgroundGranted === PermissionsAndroid.RESULTS.GRANTED) {
          Alert.alert('Permission Granted', 'Background location access enabled.');
        } else {
          Alert.alert(
            'Permission Denied',
            'Please enable "Allow all the time" from settings for full tracking.'
          );
        }
      } else {
  ...
  const setupForegroundNotification = () => {
    if (Platform.OS === 'android') {
      Roam.setForegroundNotification(
        ...
        'Tap to open app',
        'mipmap/ic_launcher',
        'com.sampleandroidalwaysallowbatchlistener',
